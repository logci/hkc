...
export const inspectRepo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const env = (appJson?.env || {}) as Record<string, AppJsonEnvVar>;
    const vars = Object.entries(env).map(([key, v]) => ({
      key,
    ...
    const formation = (appJson?.formation || {}) as Record<string, AppJsonFormation>;
    const dynos = Object.entries(formation).map(([type, v]) => ({
      type,
      quantity: v?.quantity ?? 1,
      size: v?.size || "eco",
    }));
    if (dynos.length === 0) dynos.push({ type: "web", quantity: 1, size: "eco" });
    const addonsRaw = (Array.isArray(appJson?.addons) ? appJson.addons : []) as Array<
      string | AppJsonAddon | null
    >;
...
export const getAppDetails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      dynos: (formation as HerokuFormation[]).map((f) => ({
        type: f.type || "web",
        quantity: f.quantity ?? 1,
        size: f.size || "eco",
      })),
      addons: (addons as HerokuAddon[]).map((a) => ({
        name: a.name || "",
...
export const deployRepo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await getDB()
      .prepare(
        `INSERT INTO deployed_apps (id, password, repo_url, heroku_app_name, heroku_app_url, config_vars)
                VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        appName,
        ...
        appName,
        app?.web_url || null,
        JSON.stringify(data.configVars),
      )
      .run();
...
export const getBuildStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const res = await fetch(`${HEROKU_API}/apps/${data.appName}/builds/${data.buildId}`, {
      headers: herokuHeaders(),
    });
...
export const updateAppVars = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await getDB()
      ...
      .bind(JSON.stringify(data.configVars), row.id)
      .run();
